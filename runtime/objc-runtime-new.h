...
class list_array_tt {
 ...
 private:
    ...
    /**
     数组指针通过 arrayAndFlag 记录，通过将指针低位标记为 1 标识数组存在。获取数组时将低位恢复为 0
     */
    bool hasArray() const {
    ...
    void setArray(array_t *array) {
        // a | 1: 末尾始终为1，即标记有数组（bool hasArray() 方法）
 ...
 public:
    ...
    /** category：8. 方法列表实际上是一个二维数组
        [
            [method, method, method],   // 分类1中的方法列表
            [method, method, method],   // 分类2中的方法列表
            [method, method, method],   // 类中的方法列表
        ]
    */
    void attachLists(List* const * addedLists, uint32_t addedCount) {
        ...
        if (hasArray()) {
            ...
            // array 中的 lists 指针向后移动 addedCount
        ...
        else if (!list  &&  addedCount == 1) { // 0 到 1 的时候使用 list 存储
            // 0 lists -> 1 list
            list = addedLists[0];
        } 
        else {
