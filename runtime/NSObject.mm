...
{ // location 是指针地址
 ...
 retry:
    ...
    // 也就是避免 +initialize 中调用了 storeWeak 方法，
    // 而在 storeWeak 方法中 weak_register_no_lock 方法中用到对象的 isa 还没有初始化完成的情况
    if (haveNew  &&  newObj) {
        Class cls = newObj->getIsa();
        // isa 非空但未初始化完成
        if (cls != previouslyInitializedClass  &&  
            !((objc_class *)cls)->isInitialized())
        { // Class 和上次不同并且 initialized 为完成时
            SideTable::unlockTwo<haveOld, haveNew>(oldTable, newTable);
            ...
            // 记录上一次 initialized 完成的 Class
            // previouslyInitializedClass 记录当前 Class，下次判断 Class 是否发生变化
