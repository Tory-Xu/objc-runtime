...
attachCategories(Class cls, category_list *cats, bool flush_caches)
{
    ...
    bool fromBundle = NO;
    while (i--) {
        /** 从尾部开始遍历分类，将分类中的方法列表添加到 mlists 中（二维数组），因此后面编译的分类中的方法会放在 mlists 的前端
         */
...
static void methodizeClass(Class cls)
{
    ...
    // 将 ro 中的方法列表、属性列表和协议列表信息记录到 rw 中
